...
fn vs_main(
  ...
  if (view.gather.z > 0.00001) {
    ...
    direction = safeNormalize(mix(direction, normalize(cluster - renderPosition), amount));
...
export default function AeroShards({
  ...
  return (
    ...
    >
      ...
      {errorMsg && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 text-red-500 p-4 font-mono text-sm whitespace-pre-wrap">
          WebGPU Error: {errorMsg}
        </div>
      )}
    </div>
  );
